...
class BreakoutTemplateFilterSet(NautobotFilterSet, NameSearchFilterSet):
    is_shuffle = django_filters.BooleanFilter()
    is_breakout = django_filters.BooleanFilter(method="filter_is_breakout", label="Is breakout")
...
class CableFilterSet(NautobotFilterSet, StatusModelFilterSetMixin):
    ...
    breakout_template = NaturalKeyOrPKMultipleChoiceFilter(
        queryset=BreakoutTemplate.objects.all(),
        to_field_name="name",
        label="Breakout Template (name or ID)",
    )
    has_breakout_template = RelatedMembershipBooleanFilter(
        field_name="breakout_template",
        label="Has breakout template",
    )
