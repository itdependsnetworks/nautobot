...
class Rack(PrimaryModel):
    ...
    def get_available_units(self, u_height=1, rack_face=None, exclude=None):
        ...
        """
        # Gather all devices which consume U space within the rack. Honor a prefetched `devices`
        # cache (see RackDetailTable) rather than `.filter()`, which would issue a fresh query per
        # rack when rendering the rack list's utilization column.
