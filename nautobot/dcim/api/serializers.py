...
class BreakoutTemplateSerializer(TaggedModelSerializerMixin, NautobotModelSerializer):
    class Meta:
...
class CableSerializer(TaggedModelSerializerMixin, NautobotModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.breakout_template_id:
            lanes = []
    ...
    def create(self, validated_data):
        ...
        if lanes_data and cable.breakout_template:
    ...
    def _assign_lanes(self, cable, lanes_data):
        ...
        template = cable.breakout_template
        ...
        for lane_data in lanes_data:
            ...
            if lane_num < 1 or lane_num > len(mapping):
                raise serializers.ValidationError(
                    f"Lane {lane_num} is outside the template's range (1-{len(mapping)})."
                )
