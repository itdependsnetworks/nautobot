...
def create_cablepath(node, rebuild=True):
    ...
    if cable.breakout_template_id:
        my_endpoint = CableTerminationEndpoint.objects.filter(
            cable=cable, termination_id=node.pk
        ).first()
        ...
        if my_endpoint and my_endpoint.connector is not None:
            ...
            for mapping_entry in cable.breakout_template.mapping:
                if mapping_entry[origin_side_key] == my_endpoint.connector:
                    ...
                    far_ep = CableTerminationEndpoint.objects.filter(
                        cable=cable, cable_end=opposite_side, connector=far_connector,
                    ).first()
...
def _create_cable_termination_rows(instance, logger):
    ...
    if instance.breakout_template_id and instance.breakout_template.mapping:
        entry = instance.breakout_template.mapping[0]
