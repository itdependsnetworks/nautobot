...
from nautobot.core.api.import_export_document import build_import_document, IMPORT_DOCUMENT_VERSION
...
class ImportExportJobTestCase(TransactionTestCase):
    ...
    def run_import(self, csv_data=None, *, model=Status, expected_status=JobResultStatusChoices.STATUS_SUCCESS, **kwargs):
    ...
    def assertNoIssues(self, job_result):
        self.assertFalse(
            JobLogEntry.objects.filter(job_result=job_result, log_level__in=[LogLevelChoices.LOG_WARNING, LogLevelChoices.LOG_ERROR]).exists()
        )
...
class ExportFieldSelectionTests(ImportExportJobTestCase):
    def test_select__csv(self):
        ...
        lines = self.export_lines(
            self.run_export(model=DeviceType, query_string="model=Selection+DT", export_fields="model,manufacturer__name")
        )
    ...
    def test_select__json(self):
        ...
        self.assertEqual(doc["records"], [{"model": "Selection JSON DT", "manufacturer": {"name": "Selection JSON Mfr"}}])

    def test_select__invalid(self):
        """An invalid field selection fails with a clear error naming the bad path."""
        job_result = self.run_export(export_fields="name,no_such_field", expected_status=JobResultStatusChoices.STATUS_FAILURE)
...
class MatchKeyTests(ImportExportJobTestCase):
    ...
    def test_match__nonunique_dupe_in_file(self):
        ...
        job_result = self.run_import(csv_data, match_fields="name", expected_status=JobResultStatusChoices.STATUS_FAILURE)
    ...
    def test_match__matches_multiple_existing(self):
        ...
        job_result = self.run_import(
            "name,color\ntest_multi_status1,555555", match_fields="color", roll_back_if_error=False, expected_status=JobResultStatusChoices.STATUS_FAILURE
        )
    ...
    def test_match__unknown_field(self):
        ...
        self.create_status()
        job_result = self.run_import(
            "name,color\ntest_update_status,222222", match_fields="no_such_field", expected_status=JobResultStatusChoices.STATUS_FAILURE
        )
...
class ImportErrorTests(ImportExportJobTestCase):
    ...
    def test_error__unknown_field_json(self):
        ...
        job_result = self.run_import(payload, import_format="json", expected_status=JobResultStatusChoices.STATUS_FAILURE)
    ...
    def test_error__model_mismatch(self):
        ...
        payload = json.dumps(
            {"nautobot_import": "1", "model": "dcim.device", "records": [{"name": "test_mismatch_status", "color": "111111"}]}
        )
        job_result = self.run_import(payload, import_format="json", expected_status=JobResultStatusChoices.STATUS_FAILURE)
    ...
    def test_error__unsupported_format(self):
        ...
        job_result = self.run_import("name,color\ntest_bad_format,111111", import_format="xml", expected_status=JobResultStatusChoices.STATUS_FAILURE)
...
class RollbackTests(ImportExportJobTestCase):
    ...
    def test_rollback__on_reverts_all(self):
        ...
        job_result = self.run_import(self._bad_row_csv(), roll_back_if_error=True, expected_status=JobResultStatusChoices.STATUS_FAILURE)
        log_info = JobLogEntry.objects.filter(job_result=job_result, log_level=LogLevelChoices.LOG_INFO, message__icontains="created")
    ...
    def test_rollback__off_keeps_good_rows(self):
        ...
        successes = JobLogEntry.objects.filter(job_result=job_result, log_level=LogLevelChoices.LOG_INFO, message__icontains="created")
        for idx, status_name in enumerate(("test_status1", "test_status2", "test_status3", "test_status4")):
...
class PermissionTests(ImportExportJobTestCase):
    def test_perm__export_without_permission(self):
        ...
        self.assertLog(job_result, f'User "{self.user}" does not have permission to view status objects', level=LogLevelChoices.LOG_ERROR)
    ...
    def test_perm__import_without_permission(self):
        ...
        job_result = self.run_import(self.csv_data, username=self.user.username, expected_status=JobResultStatusChoices.STATUS_FAILURE)
        self.assertLog(
            job_result, f'User "{self.user}" does not have permission to create or update status objects', level=LogLevelChoices.LOG_ERROR
        )
        self.assertFalse(Status.objects.filter(name__startswith="test_status").exists())
