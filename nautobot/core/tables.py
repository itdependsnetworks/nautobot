        ...
        **kwargs,
        ...
        Args:
            *args (list, optional): Passed through to django_tables2.Table
            columns (list, optional): Explicit list of column names to show, in order. Takes precedence
                over SavedView table config and the user's saved preferences. Used for programmatic table
                construction (and by the table N+1 audit tooling, which isolates columns one at a time).
        ...
        # An explicit `columns` argument takes precedence over any stored configuration below.
        if columns is None:
            columns = []
            _resolve_stored_columns = True
        else:
            _resolve_stored_columns = False
        if _resolve_stored_columns and is_object_embedded_search_results:
        ...
        elif _resolve_stored_columns and saved_view is not None and not table_changes_pending:
