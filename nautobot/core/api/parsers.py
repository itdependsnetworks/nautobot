...
from nautobot.core.api.import_export_document import (
...
class NautobotCSVParser(BaseParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        try:
            ...
            # Strip any leading byte-order mark, which would otherwise corrupt the first cell of the file
            # (defeating directive detection, or silently mangling the first header name).
    ...
    def _group_data_by_field_name(self, data):
        ...
        return nest_flat_dict(data, null_sentinels=(CSV_NO_OBJECT, CSV_NULL_TYPE))
...
class ImportDocumentParserMixin:
    ...
    def record_to_data(self, counter, record, serializer, strict=True):
        ...
        record = nest_flat_dict(record, null_sentinels=(CSV_NO_OBJECT, CSV_NULL_TYPE))
        data = {}
        unknown = []
